...
<div
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left">
      ...
      <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
        @forelse ($householders as $householder)
          @php
            $fee        = $householder->feeHistories->first();
            $feeAmount  = $fee?->amount ?? 0;
            $headMember = $householder->residents->first(); // only head is eager-loaded in index
            $displayName = $headMember?->fullname ?? $householder->fullname;
          ...
          <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group"
            ...
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                @if($householder->photoUrl())
                  <img src="{{ $householder->photoUrl() }}" alt="{{ $displayName }}"
                ...
                <div>
                  ...
                  <div class="flex items-center gap-2 mt-0.5">
                    ...
                    @if(($householder->residents_count ?? 0) > 0)
                      <span class="text-[10px] text-slate-400">{{ $householder->residents_count }} member{{ $householder->residents_count > 1 ? 's' : '' }}</span>
                    @endif
                  </div>
                </div>
              </div>
            ...
            <td class="px-6 py-4">
              <div class="flex items-center justify-center gap-1">
                ...
                <a href="{{ route('householders.edit', $householder) }}"
  ...
  @if($householders->hasPages())
    <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center sm:justify-between gap-3">
      <p class="text-sm text-slate-500 text-center sm:text-left">{{ __('app.showing') }} {{ $householders->firstItem() }}–{{ $householders->lastItem() }}
        {{ __('app.of') }}
        {{ $householders->total() }} {{ __('app.residents_lowercase') }}
      </p>
      <div class="flex items-center gap-1">
        @if ($householders->onFirstPage())
          <button
        ...
        @else
          <a href="{{ $householders->previousPageUrl() }}"
        ...
        @php
          $lastPage    = $householders->lastPage();
          $currentPage = $householders->currentPage();
          $start       = max(1, $currentPage - 2);
          $end         = min($lastPage, $currentPage + 2);
        @endphp

        @if ($start > 1)
          <a href="{{ $householders->url(1) }}" class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">1</a>
          @if ($start > 2)
        ...
        @for ($p = $start; $p <= $end; $p++)
          ...
          @else
            <a href="{{ $householders->url($p) }}" class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{{ $p }}</a>
          @endif
        @endfor
        ...
        @if ($end < $lastPage)
          @if ($end < $lastPage - 1)
            <span class="px-1 text-slate-400 text-sm">&hellip;</span>
          @endif
          <a href="{{ $householders->url($lastPage) }}" class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{{ $lastPage }}</a>
        @endif

        @if ($householders->hasMorePages())
          <a href="{{ $householders->nextPageUrl() }}"
            class="p-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
