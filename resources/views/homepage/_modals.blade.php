...
<script>
  ...
  const buletinUpdateUrlTpl = '{{ route('homepage.buletin.update', '__id__') }}';
  const buletinDeleteUrlTpl = '{{ route('homepage.buletin.destroy', '__id__') }}';

  function openBuletinEditModal(btn) {
    const modal = document.getElementById('buletin-edit-modal');
    const form  = document.getElementById('buletin-edit-form');
    const { id, title, date, description, url, imageUrl } = btn.dataset;
    form.action = buletinUpdateUrlTpl.replace('__id__', id);
    document.getElementById('edit-buletin-title').value       = title       || '';
    document.getElementById('edit-buletin-date').value        = date        || '';
    document.getElementById('edit-buletin-description').value = description || '';
    document.getElementById('edit-buletin-url').value         = url         || '';
    clearImageInput('edit-buletin-img-input', 'edit-buletin-img-preview', 'edit-buletin-img-label');
    const currentWrap  = document.getElementById('edit-buletin-img-current');
    const currentThumb = document.getElementById('edit-buletin-img-current-thumb');
    const currentUrlEl = document.getElementById('edit-buletin-img-current-url');
    if (imageUrl) {
      currentThumb.src         = imageUrl;
      currentUrlEl.textContent = imageUrl;
      currentWrap.classList.remove('hidden');
  ...
  function closeBuletinEditModal() {
    document.getElementById('buletin-edit-modal').classList.add('hidden');
    document.body.style.overflow = '';
  }

  function openBuletinDeleteModal(id, title) {
    const modal = document.getElementById('buletin-delete-modal');
    document.getElementById('buletin-delete-body').textContent = '{{ __('app.hp_buletin_delete_body_before') }} "' + title + '" {{ __('app.hp_buletin_delete_body_after') }}';
    document.getElementById('buletin-delete-form').action = buletinDeleteUrlTpl.replace('__id__', id);
    modal.classList.remove('hidden');
    document.body.style.overflow = 'hidden';
  }

  function closeBuletinDeleteModal() {
    document.getElementById('buletin-delete-modal').classList.add('hidden');
    document.body.style.overflow = '';
  }
</script>

{{-- ── Buletin Edit Modal ──────────────────────────────────────────────────── --}}
<div id="buletin-edit-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4"
  style="background:rgba(0,0,0,0.5);backdrop-filter:blur(4px)">
  <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg">
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
      <div class="flex items-center gap-2">
        <span class="material-icons text-sky-500 text-[20px]">edit_note</span>
        <h3 class="font-bold text-slate-900 dark:text-white text-base">{{ __('app.hp_edit_buletin_title') }}</h3>
      </div>
      <button type="button" onclick="closeBuletinEditModal()"
        class="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
        <span class="material-icons text-[20px]">close</span>
      </button>
    </div>
    <form id="buletin-edit-form" method="POST" action="" class="p-6 space-y-4" enctype="multipart/form-data" novalidate>
      @csrf @method('PUT')
      <div class="space-y-1.5">
        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">
          {{ __('app.hp_buletin_title_label') }} <span class="text-rose-500">*</span>
        </label>
        <input type="text" id="edit-buletin-title" name="title"
          class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" oninput="clearHpErr('err-hp-edit-buletin-title')">
        <p id="err-hp-edit-buletin-title" class="hidden mt-1 text-sm text-rose-500"></p>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="space-y-1.5">
          <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('app.hp_col_date') }}</label>
          <input type="date" id="edit-buletin-date" name="date"
            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all dark:[color-scheme:dark]">
        </div>
        <div class="space-y-1.5">
          <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('app.hp_desc_label') }}</label>
          <input type="text" id="edit-buletin-description" name="description"
            class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
            placeholder="Short description... (optional)">
        </div>
      </div>
      <div class="space-y-1.5">
        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('app.hp_url_label') }} <span class="text-slate-400 font-normal text-xs">{{ __('app.hp_url_hint') }}</span></label>
        <input type="url" id="edit-buletin-url" name="url"
          class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
          placeholder="https://... (optional)">
      </div>
      <div class="space-y-1.5">
        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300">{{ __('app.hp_buletin_image') }}</label>
        <div id="edit-buletin-img-current" class="hidden items-center gap-4 p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 mb-2">
          <img id="edit-buletin-img-current-thumb" src="" alt="Current" class="w-20 h-14 object-cover rounded-lg border border-slate-200 dark:border-slate-700 flex-shrink-0">
          <div class="flex-1 min-w-0">
            <p class="text-xs font-semibold text-slate-600 dark:text-slate-400">{{ __('app.hp_current_image') }}</p>
            <p class="text-xs text-slate-400 truncate" id="edit-buletin-img-current-url"></p>
          </div>
        </div>
        <label id="edit-buletin-img-label" class="flex flex-col items-center justify-center gap-2 w-full h-24 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50 hover:border-primary/60 hover:bg-primary/5 transition-all cursor-pointer">
          <span class="material-icons text-slate-400 text-2xl">cloud_upload</span>
          <span class="text-xs font-semibold text-slate-500">{{ __('app.hp_upload_new_image') }} <span class="text-slate-400 font-normal">{{ __('app.hp_upload_optional_hint') }}</span></span>
          <input type="file" name="image_file" id="edit-buletin-img-input" accept="image/*" class="sr-only"
            onchange="previewImage(this,'edit-buletin-img-preview','edit-buletin-img-label')">
        </label>
        <div id="edit-buletin-img-preview" class="hidden items-center gap-3 p-3 rounded-xl border border-primary/30 bg-primary/5">
          <img src="" alt="Preview" class="w-16 h-12 object-cover rounded-lg flex-shrink-0">
          <div class="flex-1 min-w-0">
            <p class="text-xs font-semibold text-primary">{{ __('app.hp_ready_to_upload') }}</p>
            <p class="text-xs text-slate-400 truncate"></p>
          </div>
          <button type="button" onclick="clearImageInput('edit-buletin-img-input','edit-buletin-img-preview','edit-buletin-img-label')" class="text-slate-400 hover:text-rose-500 transition-colors">
            <span class="material-icons text-lg">close</span>
          </button>
        </div>
      </div>
...
{{-- ── Buletin Delete Modal ──────────────────────────────────────────────────── --}}
<div id="buletin-delete-modal"
  class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
  <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
    <div class="flex flex-col items-center pt-8 pb-5 px-6 text-center">
      ...
      <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2">{{ __('app.hp_remove_buletin_title') }}</h3>
      <p id="buletin-delete-body" class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed"></p>
    </div>
    <div class="flex gap-3 px-6 pb-6">
      <button onclick="closeBuletinDeleteModal()"
        class="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-semibold
          text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
        {{ __('app.btn_cancel') }}
      </button>
      <form id="buletin-delete-form" method="POST" action="" class="flex-1">
        @csrf @method('DELETE')
        <button type="submit"
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (formBuletinAdd) {
      formBuletinAdd.addEventListener('submit', function(e) {
        if (!validateHpRequired('hp-buletin-title', 'err-hp-buletin-title', 'Title')) e.preventDefault();
      });
    }
  });
</script>
