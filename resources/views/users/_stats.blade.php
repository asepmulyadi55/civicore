...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
  ...
  <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      ...
      <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ __('app.total_users') }}</span>
    </div>
    <div class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($totalUsers) }}</div>
    <p class="text-sm text-slate-500 mt-1">{{ __('app.platform_users') }}</p>
  </div>
  ...
  <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      ...
      <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ __('app.active') ?? 'Active' }}</span>
    </div>
    <div class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($activeUsers) }}</div>
    <p class="text-sm text-slate-500 mt-1">{{ __('app.active_users') }}</p>
  </div>
  ...
  <div class="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      ...
      <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">{{ __('app.status_pending') }}</span>
    </div>
    <div class="text-2xl font-bold text-slate-900 dark:text-white">{{ number_format($pendingUsers) }}</div>
    <p class="text-sm text-slate-500 mt-1">{{ __('app.awaiting_approval') }}</p>
  </div>
