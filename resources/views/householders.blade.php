{{-- householders.blade.php — Orchestrator --}}
<x-layouts.app :title="__('app.nav_residents')">

  <x-nav.sidebar active="householders" />

  <div class="lg:pl-64 min-h-screen bg-background-light dark:bg-background-dark flex flex-col">
    @include('householders._header')

    <div class="flex-1 p-6 lg:p-8 space-y-6">
      @include('householders._filters')
      @include('householders._table')
    </div>
  </div>

  {{-- Householder form modals — in components/modals/ consistent with other sections --}}
  <x-modals.resident-form :blocks="$blocks" :currency="$currency" />
  @include('householders._import_modal')
  ...
  <script>
    const householdersBaseUrl = "{{ url('/householders') }}";
    const RCM_CONFIGS = {
      deactivate: {
        ...
        route: (id) => `${householdersBaseUrl}/${id}/deactivate`,
      },
      delete: {
        ...
        route: (id) => `${householdersBaseUrl}/${id}`,
      },
    };
